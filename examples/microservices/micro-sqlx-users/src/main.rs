...
async fn main() {
    ...
    grpc.add_service(crate::users::UsersGrpcService::new());
    let grpc_names = grpc.service_names();
    let manual_services = grpc.manual_len();
    tokio::spawn(async move {
        let grpc_addr = std::env::var("GRPC_ADDR").unwrap_or_else(|_| "0.0.0.0:50053".into());
        eprintln!(
            "[ferrite-grpc] users.grpc = {:?} (inventory={grpc_names:?}, manual_added={manual_services}) listening on {grpc_addr}",
            crate::users::USERS_GRPC_SERVICE_NAME
        );
