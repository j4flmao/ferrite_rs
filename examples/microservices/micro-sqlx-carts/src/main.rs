...
async fn main() {
    ...
    grpc.add_service(crate::carts::CartsGrpcService::new());
    let grpc_names = grpc.service_names();
    let manual_services = grpc.manual_len();
    tokio::spawn(async move {
        let grpc_addr = std::env::var("GRPC_ADDR").unwrap_or_else(|_| "0.0.0.0:50055".into());
        eprintln!(
            "[ferrite-grpc] carts.grpc = {:?} (inventory={grpc_names:?}, manual_added={manual_services}) listening on {grpc_addr}",
            crate::carts::CARTS_GRPC_SERVICE_NAME
        );
